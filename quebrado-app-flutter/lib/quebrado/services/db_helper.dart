...
class DatabaseHelper {
  ...
  Future<List<Map<String, String>>> loadProfiles() async {
    if (kIsWeb) {
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        final spList = await SupabaseService.instance.loadProfiles();
        if (spList.isNotEmpty) return spList;
      }
  ...
  Future<List<Account>> getAccountsForProfile(String profileId) async {
    if (kIsWeb) {
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        return await SupabaseService.instance.getAccounts(profileId: profileId);
      }
      return [];
    }
  ...
  Future<Map<String, dynamic>> getPendingDataForProfile(String profileId) async {
    if (kIsWeb) {
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        final recList = await SupabaseService.instance.getRecurringPayments(profileId: profileId);
        final confList = await SupabaseService.instance.getConfirmedPaymentKeys();
        final partList = await SupabaseService.instance.getPartials();
        return {
          'recurring': recList,
          'confirmations': confList.map((k) => {'id': k, 'recurring_payment_id': k.split('_').first, 'date': k.split('_').length > 1 ? k.split('_')[1] : ''}).toList(),
          'partials': partList,
        };
      }
      return {
        'recurring': <RecurringPayment>[],
        'confirmations': <Map<String, dynamic>>[],
        'partials': <RecurringPaymentPartial>[],
      };
    }
  ...
  Future<void> insertCrossProfileTransaction(String targetProfileId, Transaction tx, Account targetAccount) async {
    if (kIsWeb) {
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        await SupabaseService.instance.insertTransaction(tx, profileId: targetProfileId);
