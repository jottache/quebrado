...
class AppState extends ChangeNotifier {
  ...
  // MARK: - Super App Module State
  SuperAppModule _activeSuperAppModule = SuperAppModule.finances;
  SuperAppModule get activeSuperAppModule => _activeSuperAppModule;

  void setActiveSuperAppModule(SuperAppModule module) {
    if (_activeSuperAppModule != module) {
      _activeSuperAppModule = module;
      notifyListeners();
    }
  }
  ...
  // In-memory multi-profile cache for instant switching
  Map<String, List<Account>> _cachedAccounts = {};
  Map<String, List<SavingPocket>> _cachedPockets = {};
  Map<String, List<TransactionCategory>> _cachedCategories = {};
  Map<String, List<Transaction>> _cachedTransactions = {};
  Map<String, List<RecurringPayment>> _cachedRecurring = {};
  bool _isDataLoaded = false;
  bool get isDataLoaded => _isDataLoaded;
  Future<void>? _loadDataFuture;

  List<Account> getAccountsForProfile(String profileId) {
    if (profileId == _activeDbName) return accounts;
    return _cachedAccounts[profileId] ?? [];
  }
  ...
  /// Loads all settings and database tables.
  /// When using Supabase, it preloads all contabilidades/profiles concurrently into memory,
  /// allowing instant switching without additional network roundtrips.
  ...
  Future<void> _doLoadData() async {
    ...
    try {
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        // Fetch all tables across all profiles concurrently in parallel
        final results = await Future.wait([
          SupabaseService.instance.loadSettings(),
          SupabaseService.instance.loadProfiles(),
          SupabaseService.instance.getAllAccountsByProfile(),
          SupabaseService.instance.getAllPocketsByProfile(),
          SupabaseService.instance.getAllCategoriesByProfile(),
          SupabaseService.instance.getAllTransactionsByProfile(),
          SupabaseService.instance.getAllRecurringPaymentsByProfile(),
          SupabaseService.instance.getRecipients(),
          SupabaseService.instance.getMarketStores(),
          SupabaseService.instance.getMarketItems(),
          SupabaseService.instance.getMarketProducts(),
          SupabaseService.instance.getMarketTrips(),
          SupabaseService.instance.getMarketShoppingLists(),
          SupabaseService.instance.getMarketShoppingListItems(),
          SupabaseService.instance.getConfirmedPaymentKeys(),
          SupabaseService.instance.getPartials(),
        ]);

        final spSettings = results[0] as Map<String, String>;
        if (spSettings.containsKey('bcvRate')) _bcvRate = double.tryParse(spSettings['bcvRate']!) ?? _bcvRate;
        if (spSettings.containsKey('parallelRate')) _parallelRate = double.tryParse(spSettings['parallelRate']!) ?? _parallelRate;
        if (spSettings.containsKey('euroRate')) _euroRate = double.tryParse(spSettings['euroRate']!) ?? _euroRate;
        if (spSettings.containsKey('selectedCurrency')) _selectedCurrency = CurrencyType.fromString(spSettings['selectedCurrency']!);
        if (spSettings.containsKey('useBiometrics')) _useBiometrics = spSettings['useBiometrics'] == 'true';
        if (spSettings.containsKey('useSlideToConfirm')) _useSlideToConfirm = spSettings['useSlideToConfirm'] == 'true';

        final spProfiles = results[1] as List<Map<String, String>>;
        if (spProfiles.isNotEmpty) {
          _profiles = spProfiles;
          final activeP = spProfiles.firstWhere(
            (p) => p['is_active'] == 'true',
            orElse: () => spProfiles.first,
          );
          _activeDbName = activeP['id'] ?? 'quebrado.db';
        }

        _cachedAccounts = results[2] as Map<String, List<Account>>;
        _cachedPockets = results[3] as Map<String, List<SavingPocket>>;
        _cachedCategories = results[4] as Map<String, List<TransactionCategory>>;
        _cachedTransactions = results[5] as Map<String, List<Transaction>>;
        _cachedRecurring = results[6] as Map<String, List<RecurringPayment>>;

        recipients = results[7] as List<MobilePaymentRecipient>;
        marketStores = results[8] as List<MarketStore>;
        marketItems = results[9] as List<MarketItem>;
        marketProducts = results[10] as List<MarketProduct>;
        marketTrips = results[11] as List<MarketTrip>;
        shoppingLists = results[12] as List<MarketShoppingList>;
        shoppingListItems = results[13] as List<MarketShoppingListItem>;

        _confirmedKeys.clear();
        _confirmedKeys.addAll(results[14] as Set<String>);

        partialPayments = results[15] as List<RecurringPaymentPartial>;

        // Set active lists from cache
        accounts = _cachedAccounts.putIfAbsent(_activeDbName, () => []);
        pockets = _cachedPockets.putIfAbsent(_activeDbName, () => []);
        categories = _cachedCategories.putIfAbsent(_activeDbName, () => []);
        ...
        final activeProfile = _profiles.firstWhere(
          (p) => p['id'] == _activeDbName,
          orElse: () => _profiles.isNotEmpty ? _profiles.first : {'id': _activeDbName, 'name': 'Personal'},
        );
        if (activeProfile.containsKey('color') && activeProfile['color'] != null) {
          ...
          AppColors.updateThemeColor(const Color(0xFF1F6F5F));
        }
        ...
        final paraleloVal = await DatabaseHelper.instance.getSetting('parallelRate');
        ...
        final currencyVal = await DatabaseHelper.instance.getSetting('selectedCurrency');
        if (currencyVal != null) {
        ...
        _confirmedKeys.clear();
        final allConfirmations = await DatabaseHelper.instance.getAllConfirmations();
  ...
  Future<void> _ensureDefaultAccountsForActiveProfile() async {
    if (accounts.isEmpty) {
      final usdAcc = Account(
        id: "acc_usd_${DateTime.now().millisecondsSinceEpoch}",
        name: "Efectivo \$",
      ...
      final vesAcc = Account(
        id: "acc_ves_${DateTime.now().millisecondsSinceEpoch + 1}",
        name: "Banco Bs.",
      ...
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        await SupabaseService.instance.insertAccount(usdAcc, profileId: _activeDbName);
        await SupabaseService.instance.insertAccount(vesAcc, profileId: _activeDbName);
      }
      if (!kIsWeb) {
        try {
          await DatabaseHelper.instance.insertAccount(usdAcc);
          await DatabaseHelper.instance.insertAccount(vesAcc);
        } catch (_) {}
      }
    }
  }
  ...
  Future<void> toggleSelectedCurrency() async {
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.saveSetting('selectedCurrency', _selectedCurrency.name);
    }
    notifyListeners();
  }
  ...
  Future<void> setUseBiometrics(bool value) async {
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.saveSetting('useBiometrics', _useBiometrics.toString());
    }
    notifyListeners();
  }
    ...
    try {
      ...
      if (latestBcvRecorded == null || latestBcvRecorded != officialRate) {
        ...
        if (!kIsWeb) {
          ...
          rateHistory = [newRecord, ...rateHistory];
        }
      }
      ...
      if (latestEuroRecorded == null || latestEuroRecorded != euro) {
        ...
        if (!kIsWeb) {
          ...
          euroRateHistory = [newRecord, ...euroRateHistory];
        }
      }
      ...
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        await SupabaseService.instance.saveSetting('bcvRate', _bcvRate.toString());
        await SupabaseService.instance.saveSetting('parallelRate', _parallelRate.toString());
        await SupabaseService.instance.saveSetting('euroRate', _euroRate.toString());
      }
  ...
  Future<void> fetchFullRateHistory() async {
    ...
    try {
      ...
      if (!kIsWeb) {
        ...
        rateHistory = bcvRecords;
        euroRateHistory = euroRecords;
      }
  ...
  Future<void> addPocket({
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.insertPocket(pocket, profileId: _activeDbName);
    }
    if (!kIsWeb) {
  ...
  Future<void> updatePocket(SavingPocket pocket) async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.updatePocket(pocket);
    }
    if (!kIsWeb) {
      await DatabaseHelper.instance.updatePocket(pocket);
      pockets = await DatabaseHelper.instance.getPockets();
    } else {
      final idx = pockets.indexWhere((p) => p.id == pocket.id);
      if (idx != -1) pockets[idx] = pocket;
    }
    notifyListeners();
  }
  ...
  Future<void> deletePocket(String id) async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.deletePocket(id);
    }
    if (!kIsWeb) {
  ...
  Future<void> addTransaction(Transaction transaction) async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.insertTransaction(transaction, profileId: _activeDbName);
    }
    if (!kIsWeb) {
      try {
    ...
    if (transaction.destinationPocketId != null) {
      ...
      if (index != -1) {
        ...
        if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
          await SupabaseService.instance.updatePocket(pockets[index]);
        }
        if (!kIsWeb) {
          try {
            await DatabaseHelper.instance.updatePocket(pockets[index]);
  ...
  Future<void> deleteTransaction(Transaction tx) async {
    ...
    if (counterpart != null) {
      ...
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        await SupabaseService.instance.deleteTransaction(counterpart.id);
      }
      if (!kIsWeb) {
        try {
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.deleteTransaction(tx.id);
    }
    if (!kIsWeb) {
      try {
    ...
    if (!kIsWeb) {
      try {
        ...
        if (kDebugMode) {
          print("Error reloading lists after transaction deletion: $e");
        }
      }
    }
  ...
  Future<void> addCategory({
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.insertCategory(cat, profileId: _activeDbName);
    }
    if (!kIsWeb) {
  ...
  Future<void> updateCategory(TransactionCategory category) async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.updateCategory(category);
    }
    if (!kIsWeb) {
  ...
  Future<void> reorderCategories(List<TransactionCategory> reorderedList) async {
    for (int i = 0; i < reorderedList.length; i++) {
      ...
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        await SupabaseService.instance.updateCategory(reorderedList[i]);
      }
      if (!kIsWeb) {
    ...
    if (!kIsWeb) {
      ...
      categories = List.from(reorderedList);
    }
    notifyListeners();
  }
  ...
  Future<void> updateTransactionCategory(
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      final txIdx = transactions.indexWhere((t) => t.id == transactionId);
      if (txIdx != -1) {
        final updatedTx = transactions[txIdx].copyWith(categoryId: categoryId);
        await SupabaseService.instance.updateTransaction(updatedTx);
      }
    }
    if (!kIsWeb) {
      ...
      final txIdx = transactions.indexWhere((t) => t.id == transactionId);
      if (txIdx != -1) {
        transactions[txIdx] = transactions[txIdx].copyWith(categoryId: categoryId);
      }
    }
    notifyListeners();
  }
  ...
  Future<void> addRecurringPayment({
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.insertRecurringPayment(payment, profileId: _activeDbName);
    }
    if (!kIsWeb) {
  ...
  Future<void> deleteRecurringPayment(String id) async {
    final payment = recurringPayments.firstWhere((s) => s.id == id, orElse: () => recurringPayments.first);
    if (!kIsWeb) {
      await NotificationManager.shared.cancelNotification(payment);
    }

    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.deleteRecurringPayment(id);
    }
    if (!kIsWeb) {
      await DatabaseHelper.instance.deleteRecurringPayment(id);
      recurringPayments = await DatabaseHelper.instance.getRecurringPayments();
    } else {
      recurringPayments.removeWhere((p) => p.id == id);
    }
    await updatePendingPaymentsToday();
    notifyListeners();
  }
  ...
  Future<void> updateRecurringPayment(RecurringPayment payment) async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.updateRecurringPayment(payment);
    }
    if (!kIsWeb) {
      ...
      final idx = recurringPayments.indexWhere((p) => p.id == payment.id);
      if (idx != -1) recurringPayments[idx] = payment;
    }
    await updatePendingPaymentsToday();
    notifyListeners();
  }
  ...
  List<TimelineEvent> getTimelineEvents(int daysToProject, {List<SavingPocket> virtualPockets = const [], List<RecurringPayment> virtualPayments = const []}) {
        ...
        final targetAccount = accounts.firstWhere(
          ...
          orElse: () => accounts.isNotEmpty
              ...
              : Account(
                  id: accId,
                  name: 'Cuenta',
                  currency: payment.currency == CurrencyType.usd ? CurrencyType.usd : CurrencyType.bsBCV,
                  balance: 0.0,
                  colorHex: '#1F6F5F',
                  icon: 'wallet',
                ),
    ...
    for (var sim in simEvents) {
      if (sim.deposit != null) {
        ...
        final paymentList = allPayments.where((p) => p.id == partial.recurringPaymentId);
  ...
  Future<List<PendingOccurrence>> fetchPendingPaymentsForProfile(String profileId) async {
    ...
    List<RecurringPaymentPartial> partials = [];

    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      recurring = _cachedRecurring[profileId] ?? [];
      confirmedKeys = _confirmedKeys;
      partials = partialPayments;
    } else if (!kIsWeb) {
  ...
  Future<bool> confirmRecurringPayment({
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.insertConfirmation("${payment.id}_$todayStr", payment.id, todayStr);
    }
    if (!kIsWeb) {
  ...
  Future<void> registerPartialPayment({
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.insertPartial(partialObj);
    }
    if (!kIsWeb) {
  ...
  Future<void> addAccount({
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.insertAccount(acc, profileId: _activeDbName);
    }
    if (!kIsWeb) {
  ...
  Future<void> switchProfile(String dbName) async {
    ...
    final activeProfile = _profiles.firstWhere(
      (p) => p['id'] == _activeDbName,
      orElse: () => _profiles.isNotEmpty ? _profiles.first : {'id': dbName, 'name': 'Libro'},
    );
    if (activeProfile.containsKey('color') && activeProfile['color'] != null) {
      final colorHex = activeProfile['color']!;
      AppColors.updateThemeColor(Color(int.parse(colorHex.replaceFirst('#', '0xFF'))));
    } else {
      AppColors.updateThemeColor(const Color(0xFF1F6F5F)); // Default color
    }
    
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      // In-memory instant switch: zero network queries
      accounts = _cachedAccounts.putIfAbsent(_activeDbName, () => []);
      pockets = _cachedPockets.putIfAbsent(_activeDbName, () => []);
      categories = _cachedCategories.putIfAbsent(_activeDbName, () => []);
      transactions = _cachedTransactions.putIfAbsent(_activeDbName, () => []);
  ...
  Future<void> createProfile(String name, String color) async {
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.saveProfiles(id, _profiles);
    }
  }
  ...
  Future<void> updateProfile(String id, String newName, String newColor) async {
    ...
    if (index != -1) {
      ...
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        await SupabaseService.instance.saveProfiles(_activeDbName, _profiles);
      }
      if (!kIsWeb) {
  ...
  Future<void> deleteProfile(String id) async {
    ...
    _cachedAccounts.remove(id);
    _cachedPockets.remove(id);
    _cachedCategories.remove(id);
    _cachedTransactions.remove(id);
    _cachedRecurring.remove(id);
  ...
  Future<Map<String, dynamic>> loadBackupMetadata() async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      return await SupabaseService.instance.loadBackupMetadata();
    }
    if (kIsWeb) return {};
  ...
  Future<void> updateSecurityPin(String newPin) async {
    ...
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.saveBackupMetadata(metadata);
    } else if (!kIsWeb) {
  ...
  Future<List<Map<String, dynamic>>> getBackupsList() async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.checkAndPerformAutoBackup();
      return await SupabaseService.instance.listBackupSnapshots();
    }
    if (kIsWeb) return [];
  ...
  Future<void> createManualBackup() async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.performManualBackup();
    } else if (!kIsWeb) {
  ...
  Future<void> deleteBackup(String folderName) async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      await SupabaseService.instance.deleteBackupSnapshot(folderName);
    } else if (!kIsWeb) {
  ...
  Future<bool> restoreBackup(String folderName) async {
    bool ok = false;
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      ok = await SupabaseService.instance.restoreBackupSnapshot(folderName);
    } else if (!kIsWeb) {
    ...
    try {
      ...
      if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
        await SupabaseService.instance.recordRestoreHistory(
          backupName: 'Archivo importado (.json)',
          success: false,
          error: e.toString(),
        );
      } else if (!kIsWeb) {
  ...
  Future<void> exportBackupFolder(String folderName, {Rect? sharePositionOrigin}) async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      final snapshots = await SupabaseService.instance.listBackupSnapshots();
      final found = snapshots.firstWhere((s) => s['name'] == folderName, orElse: () => {});
      if (found.isNotEmpty && found['data'] != null) {
        final jsonStr = jsonEncode(found['data']);
        final xFile = XFile.fromData(
          utf8.encode(jsonStr),
          mimeType: "application/json",
          name: "$folderName.json",
        );
        await Share.shareXFiles([xFile], subject: "Copia de Seguridad Quebrado", sharePositionOrigin: sharePositionOrigin);
        return;
      }
    }
    if (!kIsWeb) {
  ...
  Future<Map<String, dynamic>> getBackupPreview(String folderPath) async {
    if (SupabaseConfig.isConfigured && SupabaseService.instance.isReady) {
      return await SupabaseService.instance.getBackupPreview(folderPath);
    }
    if (kIsWeb) return {};
